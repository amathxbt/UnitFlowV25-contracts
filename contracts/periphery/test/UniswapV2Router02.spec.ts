...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    // make a DTT<>WUSDC pair
    await fixture.factoryV2.createPair(DTT.address, WUSDC.address)
    const pairAddress = await fixture.factoryV2.getPair(DTT.address, WUSDC.address)
  ...
  async function addLiquidity(DTTAmount: BigNumber, WUSDCAmount: BigNumber) {
    await DTT.approve(router.address, MaxUint256)
    await router.addLiquidityETH(DTT.address, DTTAmount, DTTAmount, WUSDCAmount, wallet.address, MaxUint256, {
      ...overrides,
      value: WUSDCAmount
    })
  }
  ...
  it('removeLiquidityETHSupportingFeeOnTransferTokens', async () => {
    ...
    const WUSDCInPair = await WUSDC.balanceOf(pair.address)
    const liquidity = await pair.balanceOf(wallet.address)
    const totalSupply = await pair.totalSupply()
    const NaiveDTTExpected = DTTInPair.mul(liquidity).div(totalSupply)
    const WUSDCExpected = WUSDCInPair.mul(liquidity).div(totalSupply)
  ...
  describe('swapExactTokensForTokensSupportingFeeOnTransferTokens', () => {
    ...
    // WUSDC -> DTT
    it('WUSDC -> DTT', async () => {
      await WUSDC.deposit({ value: amountIn }) // mint WUSDC
      await WUSDC.approve(router.address, MaxUint256)

      await router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
        amountIn,
        0,
        [WUSDC.address, DTT.address],
        wallet.address,
        MaxUint256,
        overrides
      )
